...
def home():
    """
    Shows Home page with some informations about the database, i.e. number of users and places
...
def by_ratings(count=5, sug_count = 15):
    """
    Shows Ratings page. When GET, shows a form with a list of restaurants to add. When POST, adds an anonym user
    to the data base with the ratings given before and shows the related suggestions
...
def by_user_name( sug_count = 15):
    """
    Shows user page, dedicated to suggestions by user names. When GET, shows the form with
    the name to enter and 5 randomly picked user names from the data base. When POST, outputs the related suggestions.
...
if __name__ == '__main__':
    ...
    #In order to save time, we decided to run the flask development server online, 
    #which is not recommended for real production purposes
