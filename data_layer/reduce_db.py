...
def _reduce_reviews(collection = 'users_info'):
    """
    The function deletes every review that mention the deleted places. It deletes a
    user if it has no reviews anymore.

    Kwargs: 
        collection (str): Specify the collection of users to use. Can be 'users_info' or 'frontend_users'.
        Initial is 'users_info'.
    
    Return:
        A dictionary with two fields:  {'reviews_deleted': (int), 'users_deleted': (int)}.
    
    Raise:
        An Exception if the collection parameter is not 'users_info' or 'frontend_users'
    ...
    for u in handler.get_documents(collection):
        ...
        #if no reviews left for the user, then it has to be deleted
        if len(reviews) == 0:
