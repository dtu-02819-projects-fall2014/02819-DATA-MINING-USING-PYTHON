...
def add_ratings_to_db(values):
    ...
    Return:
        A three field dictionary: {'_id': (str), 'updated': (bool), 'inserted': (bool)}
...
def _suggestions(user):
    """
    The function gives the suggestions for a given user, by calling the recommendation algorithm.
...
def _transform_for_suggestions( user ):
    ...
    Return:
        A dictionary with one key: { <user_id (str)> : <ratings (list of dict)> }. 
        'ratings' is a list of dictionaries with one key: { <the_id_of_the_place (str)> : <rating (int)> } 
    """
